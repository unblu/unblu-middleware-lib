...
This library is used to create a middleware for the Unblu v8 platform (API v4). It provides a set of tools and utilities to facilitate the integration with Unblu services.

*NOTE:* This project is now structured as a multi-module library. See link:MIGRATION_GUIDE.md[Migration Guide] for details.

== Modules

* *unblu-middleware-lib-core* - Framework-agnostic core library (contains domain logic)
* *unblu-middleware-lib-spring* - Spring Boot integration (use this for Spring Boot projects)
* *unblu-middleware-lib-quarkus* - Quarkus integration (use this for Quarkus projects)
...
== Quarkus feature annotations

In Quarkus applications, you can explicitly enable middleware features by annotating any application class (for example your startup/service class) with:

* `@EnableUnbluMiddlewareLibCommon`
* `@EnableUnbluOutboundRequests`
* `@EnableUnbluWebhooks`
* `@EnableUnbluDialogBot`
* `@EnableUnbluConversationObservingBot`
* `@EnableUnbluExternalMessenger`
...
@Singleton
@EnableUnbluDialogBot
public class MyAwesomeMiddlewareService {
}
----
...
NOTE: For existing users, the artifact name has changed from `unblu-middleware-lib` to `unblu-middleware-lib-spring`.
...
public class MyAwesomeMiddlewareService implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        webhookHandler.onMono(eventName("conversation.new_message"), ConversationNewMessageEvent.class,
...
Use `dialogBot.accept...OfferIf()` to define conditions for accepting dialog offers. Every offer is rejected by default. Use `dialogBot.onDialog...()` methods to react on dialog events.

[source,java]
----
@Service
@RequiredArgsConstructor
@Import(UnbluDialogBot.class)
public class MyAwesomeMiddlewareService implements ApplicationRunner {
...
unblu:
  ...
  middleware:
    ...
    autoSubscribe: true # if true, the middleware will automatically subscribe to webhooks and bots after startup, otherwise you need to call webhookHandler.subscribe() and dialogBot.subscribe() manually or retrieve the Flux and subscribe to it yourself
    pingUnbluOnStartup: true # if true, the middleware will send a ping to Unblu after startup to verify connectivity; app dies if ping fails

  webhook:
    secret: another-secure-secret  # mandatory if webhooks are used
    cleanPrevious: false # see below, this is the default
    eventNames:    # useful to specify but not needed - event names passed to handlers are registered on the fly
  ...
  outboundRequests:
    ...
    apiPath: /outbound   # api path used by the middleware outbound controller, e.g. https://where.my.middleware.is.running/outbound. /outbound this is the default

  bot:
    person:
      sourceId: concierge-bot-person-id    # concierge is used as the default bot
      firstName: Unblu
      lastName: Concierge
...
Outbound requests are implicitly imported with `@Import(UnbluDialogBot.class)` but if you don't use that annotation, you can import them explicitly with `@Import(UnbluOutboundRequests.class)`. This exposes `outboundHandler` (new standard) and `outboundRequestHandler` (compatibility) beans.

Outbound requests are handled similarly to webhooks, however require a proper response in the form of a class `<Xxx>Response` for each `<Xxx>Request`. Like with webhooks, the general practice is to respond as quickly as possible, and perform longer processing asynchronously. For outbound requests however, this may not always be possible, because an actual response with results of the handler operation is sometimes needed. For that reason, `outboundRequestHandler.on(...)` supports synchronous response handlers, and `outboundRequestHandler.onWrappedMono(...)` supports reactive response/async handlers.
...
outboundRequestHandler.onWrappedMono(
        ...
        OutboundRequestHandlerOptions.<BotDialogOpenRequest>requestOrderSpec(
...
WARNING: Jars on the jitpack repositories are not immutable (like on a SNAPSHOT respository) and the build of the jar is delegated to an external service.
